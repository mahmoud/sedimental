...
Indefinitely stuck at 0.0.1 or worse. What options and directions
...
Semantic Versioning, or [SemVer][semverorg], is currently the go-to
versioning system for recent open source software. Take a quick look
at the [40 most recent updates on the Python Package Index][pypi40]
...
We've seen assumptions about quality made based on version
number. SemVer supports this opaque comparison, punishing libraries
that get it right on the first try.

### SemVer blocks releases

More damaging than the fatuous 2.0 is the [Zeno's][zeno_paradox]
1.0. There is an epidemic.
...
### SemVer is uncertifiable

To appeal to engineering aesthetics SemVer is presented as a
...
SemVer's promise. The [remediation][semver_rerelease] does *not*
...
what do we expect from our versions? There are three primary
expectations driving modern software versioning:

## Versions go up

The later the release, the greater the version. Sofware should not
change without a version change, and that change should always be in a
positive direction.

## Versions correlate to the quality of the software
...
## Versions are numeric, except when they're not

Here's where things get hairy. Numeric versions are the default, but
non-numeric versions and version components abound.

Even non-developers understand suffixes like "alpha", "beta", "dev",
"nightly", "stable", and so on. There are also named project versions,
like those used in Linux distributions, such as Debian's "jessie" or
Ubuntu's "trusty".
...
Furthermore, as development continues, increasingly large major
versions blur together. The calendar is one common area where numbers
increase and cycle regularly, yet we're ingrained with a natural
...
Many might not realize it, but the ubiquitous Ubuntu is in fact
calendar versioned. For example, version 15.04 came out in April, 2015.
It gets better when you remember Long-Term Support. Ubuntu's LTS is 5
years. So Ubuntu's 14.04 LTS will be in 2019. You don't have
to look anything up. It's all right there in the CalVer semantics.
...
future. Protect it from the learned expectation of 2.0 or death.
...
[^2]: I've actually been saying something similar, but a little less strict, for a long time:
...
[^3]: Here are some more resources for those interested in the
      Firefox version switch saga:

      - [Firefox Rapid Release Criticism](http://www.pcworld.com/article/224842/why_firefox_rapid_release_schedule_is_a_bad_idea.html)
      ...
      Versions matter. They're part of your product's identity. Design
